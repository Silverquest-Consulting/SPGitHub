...
class Mdata extends CI_Model{
    ...
    public function lookup($data){	// this is for admin/people
    	
    	
    	$query=$this->db->query("SELECT `user_firstname`, `user_lastname`, `user_email`, `user_address1`, `user_address2`, `user_suburb`, `user_postcode`, `user_phone`  FROM `user` WHERE `".$data['type']."` LIKE '%".$data['info']."%' ");
    	return $query->result_array();
    }
    ...
    public function getReport($data){	// this is for admin/people

    	$sql = "SELECT `or_date`, `or_orderid` , `or_constituentname` , `or_address` , `or_suburb` , `or_postcode` , `or_state` , `or_email` , `or_phone` , `or_paymentmethod` , `or_cardholdername` , `or_gift_category` , `or_gift_item` , `or_gift_id` , `or_gift_qty` , `or_gift_unitprice` , `or_gift_amount`, `or_receipt`
  				FROM `sp_order`
  				WHERE `or_date` >= '".$data['from']."' AND `or_date` <= '".$data['to']."'";
    	// echo $sql;
    	$query=$this->db->query($sql);
    	return $query->result_array();
    }
    ...
    public function isExist_user($data){
		...
		//MD-20151210- make email lower case as email should be unique
    ...
    public function isExist_Admin($data){
    	//echo $data; die('die is called');
    	...
    	$pwd_hashed = hash('sha256', $password . $username);		// MA Hoa Pass from Marc, need when up to server
    	
    	//$sql = "SELECT * FROM `user` WHERE `user_email` = '".$data['username']."' AND `user_password` = '".$data['password']."' AND `user_type` = 'A' ";	//localhost
    	$sql = "SELECT * FROM `user` WHERE `user_email` = '".$data['username']."' AND `user_password` = '".$pwd_hashed."' AND `user_type` = 'A' ";
    		//echo $sql; die('die is called');
    	$query=$this->db->query($sql);
    	return $query->num_rows();
    ...
    public function insert_user($data){
		...
		$data['userInfo']['user_email'] = $email;
	...
	public function insert_order($data) {
		...
		//2015-12-06 MD-ADDED for Guest
		else {
			$_firstName =  $data[ "card" ][ "GuestFirstName" ];
			$_lastName = $data[ "card" ][ "GuestLastName" ];
			$_email = $data[ "card" ][ "GuestEmail" ];
		}
